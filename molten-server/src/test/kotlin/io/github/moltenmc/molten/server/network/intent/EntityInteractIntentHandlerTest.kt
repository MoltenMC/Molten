...
class EntityInteractIntentHandlerTest {
    @Test
    fun handlesEntityInteractIntent() {
        val handler = EntityInteractIntentHandler()
        val commandBuffer = EcsCommandBuffer()
        val context = IntentHandlerContext(commandBuffer)
        
        val intent = ServerIntent.EntityInteract(
            sourceEntityId = EntityId.of(1, generation = 0, EntityKind.PLAYER),
            routing = IntentRouting(WorldId(UUID(0, 1)), RegionPos(0, 0)),
            targetEntityId = EntityId.of(2, generation = 0, EntityKind.MOB),
        )
